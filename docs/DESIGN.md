...
`tools/fake_chip.py` advertises the hypothesised service and plays packets at the watch. Three modes:

```bash
fake_chip.py                                       # one rep per Enter
fake_chip.py --synthetic --reps 60 --course S:0;L:30;L:60;F:100
fake_chip.py --replay captures/2026-01-01-fx/cap.tsv
```

`--synthetic` generates reps from a course and a speed, varying the speed slightly per rep — sixty byte-identical reps would let a decoder bug that keys off "the value changed" pass a sixty-rep soak. `--replay` reads the same TSV `tools/decode_capture.py` reads (and the watch's own **Dump capture** prints) and reproduces the original inter-packet gaps, so the watch sees the chip's real fragmentation timing rather than a burst as fast as the loop runs.

**Linux and BlueZ only.** macOS cannot do this at all: CoreBluetooth will not let a process advertise a custom GATT service with a chosen UUID, so the watch has nothing to discover. Use a Linux box or a Raspberry Pi.

Everything in the script except the advertising itself is pure and covered by `tools/tests/test_fake_chip.py`. The packet layout there is the mirror of `FreelapProtocol.expectedLength`/`decodeMessage` and **the two must change in the same commit** — otherwise the fake chip keeps agreeing with itself while the watch decodes real packets wrongly. `source-test/ProtocolVectorTest.mc` is what makes that concrete: literal bytes generated by `fake_chip.py`, decoded by the watch's own decoder, asserted against the crossings the generator was asked for.
